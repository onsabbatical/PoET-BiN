...
MAT_weights = np.load('weight_matrix.npy')  #### no_of_class * LUT inputs, (10 * 6)#####
...
####Random Prints###
print(format(int(MAT_labels[1,4]),'#010b'))
print(format(int(MAT_labels[1,59]),'#011b'))
print(format(int(MAT_labels[1,63]),'#010b'))
...
### The integers are converted to their binary form 10,64,8###
...
for i in range(10):
	for j in range(8):
		f.write('C_'+str(i)+'_B_'+str(j)+'_inst : LUT6 generic map(INIT => "')  ###LUT 8###
		...
		for k in range(np.shape(MAT_weights)[1]):
			f.write(', I' + str(k) + ' =>  C_'+str(i*6 + k)+'_out') ###Inputs Linearly###
		f.write('); \n')
	f.write('\n');
