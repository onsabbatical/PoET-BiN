...
cfg = {
    'VGG11': [64, 'M', 128, 'M', 256, 256, 'M', 512, 512, 'M', 512, 'L', 'M', 'B'],
...
class VGG(nn.Module):
    def __init__(self, vgg_name,inputs=8):
        ...
        self.inputs=inputs
        self.layer_int0 = nn.Linear(512,4096)
        self.batch0 = nn.BatchNorm1d(4096)
        self.layer_int1 = nn.Linear(4096,4096)
        self.batch1 = nn.BatchNorm1d(4096)
        self.layer_int2 = nn.Linear(4096,inputs*10)
        self.batch2 = nn.BatchNorm1d(inputs*10)
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x == 'M':
                ...
                layers += [Conv2d(in_channels, 512, kernel_size=3, padding=1),
                           nn.BatchNorm2d(512)]
            elif x == 'B':
                layers += [bin_act()]
            else:
